        ...
        <nav className="space-y-2">
          {menuItems.map((item) => (
            item.label !== "Help & Support" ? (
              <button
                key={item.label}
                className="flex w-full items-center gap-4 rounded-xl px-4 py-4 hover:bg-gray-100 transition cursor-pointer"
              >
                <span>
                  <item.icon />
                </span>
        ...
        <div className="absolute bottom-6 left-6 right-6 space-y-3">
          ...
          {user ? (
            <button
              ...
              className="w-full rounded-full border border-gray-400 py-2 cursor-pointer hover:bg-gray-50"
            >
              Log out
            </button>
          ) : (
