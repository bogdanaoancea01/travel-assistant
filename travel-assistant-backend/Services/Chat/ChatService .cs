    ...
    public class ChatService : IChatService
    {
        ...
        private const string TripSystemPrompt = """
            ...
            4. SCHEMA
               ...
               - placeName must be a single, exact, real-world location that resolves to ONE point on a map — a specific named landmark, building, square, museum, or street (e.g. "Colosseum", "Strada Lipscani", "Piazza Navona"). It is used to drop the map pin, so it must be unambiguous and a real place that exists by that name. NEVER put vague, experiential, or aggregate names here (no "Old Town Walk", "City Tour", "Beach Day", "Dinner downtown"). If the activity is a walk or covers an area, choose the single most iconic real landmark or named street that best represents it. Never include diacritics or special characters. (Exception: dining activities — see below.)
               - DINING ACTIVITIES (dinner, lunch, breakfast, food experiences, drinks): do NOT recommend or name a specific restaurant, bar, or cafe. Instead, set isDining: true, set name to an experiential label (e.g. "Old Town dinner", "Riverside lunch"), and set placeName to the real, named neighbourhood, district, or square best suited for that meal (e.g. "Old Town, Sibiu", "Piata Mica, Sibiu", "Trastevere, Rome"). Never put a single venue in placeName for dining — the map will show the restaurant options across that area for the user to choose from. The description should describe the dining scene/vibe of the area, not a particular establishment.
               - For non-dining activities, set isDining: false.
               - For each activity, provide a full address in the format "Street, City, Country" — be as precise as possible. For dining activities, give the address of the neighbourhood/area (street or square level) rather than a specific venue. Never include diacritics or special characters in the address.
